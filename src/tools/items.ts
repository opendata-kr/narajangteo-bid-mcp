...
export const itemsInputShape = {
  ...
  bidKind: z.enum(["thng", "servc", "frgcpt"]).optional()
    .describe("업무구분(thng=물품, servc=용역, frgcpt=외자). 공사는 구매대상물품 없음. 미지정 시 3구분 병렬"),
};
export type ItemsArgs = { bidNtceNo: string; bidNtceOrd?: string; bidKind?: ItemKind };

export interface ItemsResult {
  bidNtceNo: string; bidNtceOrd: string; anySucceeded: boolean;
  results: Record<string, OpOutcome<BidItem>>;
}
...
export async function runItems(client: DataGoKrClient, args: ItemsArgs): Promise<ItemsResult> {
  ...
  const kinds = (args.bidKind ? [args.bidKind] : ITEM_KINDS) as ItemKind[];
  ...
  const { results, anySucceeded } = await runOps(
    client, kinds.map((k) => ({ label: k, op: ITEM_OP[k], params })), formatItemRow,
  );

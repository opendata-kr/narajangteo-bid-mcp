import { z } from "zod";
import { withKeyHint, errMessage, type DataGoKrClient } from "@opendata-kr/core";
...
async function resolveNoticeSpec(
  ...
  const settled = await Promise.allSettled(
    NOTICE_KINDS.map((k) => client.call(listOperation(k), { ...params })),
  );
  const errors: string[] = [];
  let rejected = 0;
  for (const s of settled) {
    if (s.status === "fulfilled" && s.value.items.length > 0) {
      const items = formatNoticeSpecAttachments(s.value.items[0]!);
      return { status: "ok", totalCount: items.length, items };
    }
    if (s.status === "rejected") {
      rejected += 1;
      const m = withKeyHint(client, errMessage(s.reason));
  ...
  return { status: "ok", totalCount: 0, items: [] };
}
...
export async function resolveAttachments(
  ...
  const [opResolved, notice] = await Promise.all([
    runOps(client, calls, formatAttachment),
    resolveNoticeSpec(client, bidNtceNo),
  ]);
  const results: Record<string, OpOutcome<BidAttachment>> = { notice, ...opResolved.results };

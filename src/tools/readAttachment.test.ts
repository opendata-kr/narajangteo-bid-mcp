...
let tmp: string;
beforeAll(() => {
  tmp = mkdtempSync(path.join(os.tmpdir(), "read-attach-"));
...
describe("runReadAttachment", () => {
  it("최상위 hwpx를 index로 읽어 본문을 반환한다", async () => {
  ...
  it("zip 내부 파일을 index로 직접 읽는다(container 표기)", async () => {
  ...
  it("미지원(extractable=false) 항목은 unsupported·빈 텍스트", async () => {
  ...
  it("index 범위 초과는 회복 지시 에러로 throw한다", async () => {
  ...
  it("offset·maxChars로 페이지네이션한다", async () => {
    const long = "가".repeat(5000);
    const client = makeClient([["긴문서.hwpx", "https://f/l"]]);
    const fetchFn = makeFetch({ "https://f/l": makeHwpx(long) });
    const mid = await runReadAttachment(
      client,
      { bidNtceNo: "RA5", index: 0, offset: 1000, maxChars: 1000 },
      { fetch: fetchFn },
    );
    ...
    const end = await runReadAttachment(
      client,
      { bidNtceNo: "RA5", index: 0, offset: 4500, maxChars: 1000 },
      { fetch: fetchFn },
    );
    expect(end.text).toHaveLength(500);
    expect(end.truncated).toBe(false);
  });

  it("download_attachments 목록의 index와 read_attachment가 일관된다", async () => {
    ...
    // 0=표지.hwpx, 1=묶음.zip/a.hwpx, 2=묶음.zip/b.doc

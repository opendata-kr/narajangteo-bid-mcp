...
describe("loadConfig", () => {
  it("DATA_GO_KR_SERVICE_KEY를 읽는다", () => {
    const c = loadConfig({ DATA_GO_KR_SERVICE_KEY: "abc123" });
  ...
  it("키가 없으면 명확한 에러를 던진다", () => {
    expect(() => loadConfig({})).toThrow(/DATA_GO_KR_SERVICE_KEY/);
  });

  it("이미 URL 인코딩된 키(%포함)는 그대로 두되 경고 대상으로 감지한다", () => {
    const c = loadConfig({ DATA_GO_KR_SERVICE_KEY: "abc%2Bdef" });
